...
describe('TaskCompletionService', () => {
	...
	it('calls updateTasks with an updater that replaces the completed task', async () => {
		const task = makeTask({ id: 'task-1' });
		const completed = makeTask({ id: 'task-1', completedAt: new Date().toISOString() });
		mockApi.completeTask.mockResolvedValue(completed);
	...
	it('calls undoCompletion and updateTasks when undo action is triggered', async () => {
		const task = makeTask();
		...
		// Trigger the undo action from the toast
		const config: ToastConfig = mockToastService.AddToast.mock.calls[0][0];
