...
Then(
	...
	async ({ featureFlagsPage }, heading: string) => {
...
Then(
	'I should see a flag row for {string}',
	async ({ featureFlagsPage }, name: string) => {
...
Then(
	'the toggle for {string} should be off',
	async ({ featureFlagsPage }, name: string) => {
...
When(
	'I toggle the switch for {string} on',
	async ({ featureFlagsPage }, name: string) => {
...
Then(
	'the toggle for {string} should be on',
	async ({ featureFlagsPage }, name: string) => {
		const on = await featureFlagsPage.isToggleOn(name);
		expect(on).toBe(true);
	}
...
When(
	'I toggle the switch for {string} off',
	async ({ featureFlagsPage }, name: string) => {
		await featureFlagsPage.toggleFlag(name);
	}
);
